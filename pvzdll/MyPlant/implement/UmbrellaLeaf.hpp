	...
	class UmbrellaLeaf : public NoEasterSkinPlant
	{
		...
		void TickPassive(MyPlant plant)
		{
			...
			for (auto zombie : zombies)
				if (zombie.Row == plant.Row && !zombie.Blowaway && (zombie.State == ZombieState::POLE_VALUTING_JUMPPING
						...
						|| zombie.State == ZombieState::IMP_FLYING))
					if (plant.ImageX - zombie.X < 80 && plant.ImageX - zombie.X > 160)
					{
						zombie.Blowaway = 1;
						zombie.Layer = 0x61A80 + zombie.Row * 10000;
						if (zombie.State == ZombieState::IMP_FLYING)
						{
							PVZ::Memory::WriteMemoryUnsafe<float>(zombie.GetBaseAddress() + 0x120, 5.0f);
							zombie.Speed *= -2;
						}
